...
def run_study(problem_name: str = "Sphere", env_name: str = "Traditional", n_trials: int = 30) -> pd.DataFrame:
    ...
    def objective(trial):
        ...
        gamma_ba = trial.suggest_float("gamma_ba", 0.5, 2.0)
    ...
    print(f"[Optuna MPBBA] Best for {problem_name}/{env_name}: {study.best_trial.params}")
...
def main():
    ...
    out_path = os.path.join(output_dir, "robust_optuna_catalog_mpbba.csv")
