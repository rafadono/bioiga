...
class FitnessStrategy(ABC):
    ...
    def __init__(self, problem: MathProblem, config: MPBGWOConfig):
        self.problem = problem
        self.config = config

    @abstractmethod
    def evaluate(self, wolf: Wolf, current_gen: int) -> Tuple[float, float, float]:
        """
        Evaluate a wolf's fitness.
        """
...
class TraditionalEnv(FitnessStrategy):
    ...
    def evaluate(self, wolf: Wolf, current_gen: int) -> Tuple[float, float, float]:
        youth_err = self.problem.evaluate_partial(wolf.get_youth_genes())
        late_err = self.problem.evaluate_partial(wolf.get_late_genes())
