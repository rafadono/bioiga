...
class MPGAAlgorithm:
    def __init__(self, config: MPGAConfig, fitness_strategy: FitnessStrategy):
        ...
        self.islands = [[Individual(config) for _ in range(config.pop_size)] for _ in range(self.num_islands)]
    ...
    def _apply_mortality_modules(self, population: List[Individual]) -> List[Individual]:
        ...
        if len(survivors) < self.config.tournament_size:
            survivors.extend([Individual(self.config) for _ in range(self.config.tournament_size - len(survivors))])
    ...
    def _crossover(self, parent1: Individual, parent2: Individual) -> Tuple[Individual, Individual]:
        ...
        if random.random() > self.config.crossover_rate:
            return Individual(self.config, np.copy(parent1.genes)), Individual(self.config, np.copy(parent2.genes))
