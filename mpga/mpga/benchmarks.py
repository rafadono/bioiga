...
class MathProblem(ABC):
    ...
    def evaluate_partial(self, genes: np.ndarray) -> float:
        """Evaluate the function on a subset of continuous genes."""
...
class FitnessStrategy(ABC):
    ...
    def __init__(self, problem: MathProblem, config: MPGAConfig):
        self.problem = problem
        self.config = config

    @abstractmethod
    def evaluate(self, individual: Individual, current_gen: int) -> Tuple[float, float, float]:
        """
...
class TraditionalEnv(FitnessStrategy):
    ...
    def evaluate(self, individual: Individual, current_gen: int) -> Tuple[float, float, float]:
        youth_err = self.problem.evaluate_partial(individual.get_youth_genes())
        late_err = self.problem.evaluate_partial(individual.get_late_genes())
