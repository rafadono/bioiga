...
def plot_results(
    ...
    ax2.plot(last_history["gen"], last_history["youth_error"],
             label="Youth genes", color="royalblue", lw=1.8)
    ax2.plot(last_history["gen"], last_history["late_error"],
             label="Late genes", color="tomato", lw=1.8, linestyle="--")

    if hasattr(config, "asteroid_gen"):
        ax2.axvline(config.asteroid_gen, color="gray", linestyle=":",
                    lw=1.5, label=f"Asteroid (gen {config.asteroid_gen})")
