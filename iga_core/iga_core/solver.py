...
class IGASolver:
    ...
    def _precompute_element_matrices(self, geometry):
        ...
        for gp_x in gauss_pts:
            for gp_y in gauss_pts:
                ...
                dN_deta = 0.25 * np.array([-(1.0 - gp_x), -(1.0 + gp_x), (1.0 + gp_x), (1.0 - gp_x)])
                N = 0.25 * np.array([(1.0 - gp_x) * (1.0 - gp_y), (1.0 + gp_x) * (1.0 - gp_y), (1.0 + gp_x) * (1.0 + gp_y), (1.0 - gp_x) * (1.0 + gp_y)])
        ...
        for i in range(num_u - 1):
            for j in range(num_v - 1):
                ...
                x_coords = np.array([P[i, j, 0], P[i+1, j, 0], P[i+1, j+1, 0], P[i, j+1, 0]])
                y_coords = np.array([P[i, j, 1], P[i+1, j, 1], P[i+1, j+1, 1], P[i, j+1, 1]])
    ...
    def assemble_system(self, geometry, densities, build_mass=False):
        ...
        k_rows, k_cols, k_vals, m_rows, m_cols, m_vals = iga_rust.assemble_precomputed_rust(
            num_u, num_v, Ke_flat, Me_flat, dofs_flat, densities_flat, build_mass,
            self.kernel.E0, self.kernel.rho0, self.kernel.p
        )
    ...
    def solve_bloch_frequencies(self, K, M, kx, ky, num_u, num_v, num_modes=6, K_is_culled=False):
        ...
        from scipy.sparse import lil_matrix, eye, kron, vstack
        ...
        try:
            eigenvalues = eigsh(K_bloch, k=num_modes, M=M_bloch, sigma=1e-3, which='LM', return_eigenvectors=False)
