...
from .domain import (
    StructuralDesign,
    ensure_feasibility_and_symmetry,
    extract_independent_variables,
...
class IGAOptimizer:
    ...
    def _evaluate_fitness(self, design, objective_type="compliance", load_case=None):
        ...
        if objective_type == "bandgap":
            ...
            vol_penalty = (
                (np.mean(design.densities) - config.target_volume) * 1e9
                if np.mean(design.densities) > config.target_volume
                else 0.0
            )
            ...
            if strategy == "legacy":
                ...
                if not hasattr(self, "base_compliance") or self.base_compliance is None:
                    ...
                    von_mises_base = solver.kernel.compute_von_mises_stress(
                        design.geometry, solid_densities, U_base
                    )
                    p = getattr(config, "p_norm_exponent", 8)
                    self.base_stress = (np.mean((von_mises_base / sigma_adm) ** p)) ** (
                        1.0 / p
                    ) * sigma_adm
                    print(
                        f"[Strategy 2] Base Case Reference Calculated: C0 = {self.base_compliance:.4e}, S0 = {self.base_stress:.4e}"
                    )
    ...
    def optimize(self, initial_geometry, strategy="topology", void_mask=None, solid_mask=None):
        ...
        self.population = [
            StructuralDesign(initial_geometry, void_mask, solid_mask)
    ...
    def optimize_mpga(
        ...
        if use_symmetry:
            if num_u % 2 != 0 or num_v % 2 != 0:
                raise ValueError(
                    "The grid dimensions num_u and num_v must be even to apply symmetry."
                )
        ...
        print(
            f"--- Starting MpGA | {num_pop} Populations | Heterogeneous: {heterogeneous} | Topology: {migration_topology} ---"
        )
        ...
        for p_idx in range(num_pop):
            for idx in range(self.config.pop_size):
                ...
                chromosomes[p_idx][idx] = self._update_evaluate_and_repair(
                    populations[p_idx][idx], chromosomes[p_idx][idx], num_u, num_v, use_symmetry
                )
        ...
        for p_idx in range(num_pop):
            if island_configs[p_idx]["type"] == "MBPSO":
                ...
                swarm_velocities[p_idx] = [
                    np.random.uniform(-5.0, 5.0, size=shape) for _ in range(self.config.pop_size)
                ]
        ...
        for gen in range(self.config.generations):
                ...
                for i in range(num_pop):
                    if migration_topology == "ring":
                        ...
                        if i == 0:
                            ...
                            source_idx = others[
                                np.argmax([populations[idx][0].fitness for idx in others])
                            ]
                        else:
                            source_idx = 0
                    ...
                    for k_rate in range(min(migration_rate, self.config.pop_size // 2)):
                        ...
                        if replacement_policy == "worst":
                            ...
                            target_idx = np.random.randint(
                                self.config.pop_size // 2, self.config.pop_size
                            )

                        chromosomes[i][target_idx] = np.copy(migrant_chrom)
                        populations[i][target_idx].stress_strategy = island_configs[i][
                            "stress_strategy"
                        ]
                        chromosomes[i][target_idx] = self._update_evaluate_and_repair(
                            populations[i][target_idx],
                            chromosomes[i][target_idx],
                            num_u,
                            num_v,
                            use_symmetry,
                        )
            ...
            for p_idx in range(num_pop):
                ...
                if cfg["type"] == "GA":
                    ...
                    next_pop = [
                        copy.deepcopy(populations[p_idx][0]),
                        copy.deepcopy(populations[p_idx][1]),
                    ]
                    ...
                    while len(next_chrom) < self.config.pop_size:
                        ...
                        repaired_c_chrom = self._update_evaluate_and_repair(
                            child, c_chrom, num_u, num_v, use_symmetry
                        )
                    ...
                    for i in range(self.config.pop_size):
                        ...
                        swarm_velocities[p_idx][i] = (
                            w_coef * swarm_velocities[p_idx][i]
                            + c1 * r1 * (swarm_pbests_x[p_idx][i] - chromosomes[p_idx][i])
                            + c2 * r2 * (swarm_gbest_x[p_idx] - chromosomes[p_idx][i])
                        )
                        swarm_velocities[p_idx][i] = np.clip(
                            swarm_velocities[p_idx][i], -v_max, v_max
                        )

                        # Map to flip probability
                        T, is_abs = self._apply_transfer_function(
                            swarm_velocities[p_idx][i],
                            cfg["tf_type"],
                            cfg["is_time_varying"],
                            gen,
                        ...
                        child = StructuralDesign(initial_geometry, void_mask, solid_mask)
                        child.stress_strategy = cfg["stress_strategy"]
                        repaired_x = self._update_evaluate_and_repair(
                            child, new_x, num_u, num_v, use_symmetry
                        )
    ...
    def optimize_mbpso(
        ...
        start_time = time.time()
        print(
            f"--- Starting MBPSO | Swarm Size: {self.config.pop_size} | TF: {tf_type} | TV: {is_time_varying} ---"
        )
        ...
        stopped_at_gen = None
        for i in range(self.config.pop_size):
            swarm_x[i] = self._update_evaluate_and_repair(
                swarm[i], swarm_x[i], num_u, num_v, use_symmetry
            )
        ...
        for gen in range(self.config.generations):
            for i, p in enumerate(swarm):
                swarm_x[i] = self._update_evaluate_and_repair(
                    p, swarm_x[i], num_u, num_v, use_symmetry
                )
            ...
            print(
                f"Gen {gen:03d} | MBPSO GBest: {gbest.compliance:.4e} | Fitness: {gbest.fitness:.3f}"
            )
            ...
            for i in range(self.config.pop_size):
                ...
                T, is_abs = self._apply_transfer_function(
                    velocities[i], tf_type, is_time_varying, gen, self.config.generations
                )
                u = np.random.rand(*shape)
    ...
    def optimize_msmbpso(
        ...
        print(
            f"--- Starting MS-MBPSO | {num_pop} Swarms | TF: {tf_type} | TV: {is_time_varying} | Topology: {migration_topology} ---"
        )
        ...
        for p_idx in range(num_pop):
            ...
            for i in range(self.config.pop_size):
                ...
                swarm_x[p_idx][i] = self._update_evaluate_and_repair(
                    swarms[p_idx][i], swarm_x[p_idx][i], num_u, num_v, use_symmetry
                )
        ...
        for gen in range(self.config.generations):
            ...
            if gen > 0 and gen % migration_interval == 0:
                print(
                    f"Gen {gen:03d} | Performing gbest migration between swarms (Topology: {migration_topology})..."
                )
                for p_idx in range(num_pop):
                    if migration_topology == "ring":
                        ...
                        if p_idx == 0:
                            others = list(range(1, num_pop))
                            source_idx = others[
                                np.argmax([g.fitness for idx, g in enumerate(gbests) if idx != 0])
                            ]
                        else:
                            source_idx = 0
                    ...
                    for _k_rate in range(min(migration_rate, self.config.pop_size // 2)):
                        if replacement_policy == "worst":
                        ...
                        swarms[p_idx][target_idx].stress_strategy = island_configs[p_idx][
                            "stress_strategy"
                        ]
                        swarm_x[p_idx][target_idx] = self._update_evaluate_and_repair(
                            swarms[p_idx][target_idx],
                            swarm_x[p_idx][target_idx],
                            num_u,
                            num_v,
                            use_symmetry,
                        )
            ...
            for p_idx in range(num_pop):
                for i, p in enumerate(swarms[p_idx]):
                    p.stress_strategy = island_configs[p_idx]["stress_strategy"]
                    swarm_x[p_idx][i] = self._update_evaluate_and_repair(
                        p, swarm_x[p_idx][i], num_u, num_v, use_symmetry
                    )
            ...
            for p_idx in range(num_pop):
                for i in range(self.config.pop_size):
                    ...
                    T, is_abs = self._apply_transfer_function(
                        velocities[p_idx][i], tf_type, is_time_varying, gen, self.config.generations
                    )
                    u = np.random.rand(*shape)
    ...
    def optimize_hybrid(
        ...
        print(
            f"--- Starting Hybrid Scheme: Stage 1 (MS-MBPSO: {stage1_gens} gens) + Stage 2 (MpGA: {stage2_gens} gens) ---"
        )
        print(
            f"--- Populations: {num_pop} | Size: {self.config.pop_size} | Seeding: {seed_pct * 100:.1f}% ---"
        )
        ...
        for p_idx in range(num_pop):
            for idx in range(self.config.pop_size):
                ...
                swarm_x[p_idx][idx] = self._update_evaluate_and_repair(
                    swarms[p_idx][idx], swarm_x[p_idx][idx], num_u, num_v, use_symmetry
                )
        ...
        for gen in range(stage1_gens):
                ...
                for p_idx in range(num_pop):
                    if migration_topology == "ring":
                        ...
                        if p_idx == 0:
                            others = list(range(1, num_pop))
                            source_idx = others[
                                np.argmax([g.fitness for g in gbests if g is not gbests[0]])
                            ]
                        else:
                            source_idx = 0
                    ...
                    swarms[p_idx][worst_idx].stress_strategy = island_configs[p_idx][
                        "stress_strategy"
                    ]
            ...
            for p_idx in range(num_pop):
                for i in range(self.config.pop_size):
                    ...
                    T, is_abs = self._apply_transfer_function(
                        velocities[p_idx][i], tf_type, is_time_varying, gen, stage1_gens
                    )
                    u = np.random.rand(*shape)
        ...
        for chrom, _fit in all_candidates:
        ...
        for gen in range(stage2_gens):
            ...
            if gen > 0 and gen % migration_interval == 0:
                print(
                    f"Gen Hibrido Stage 2 {gen:03d} | Performing migration (Topology: {migration_topology})..."
                )
                for i in range(num_pop):
                    if migration_topology == "ring":
                        source_idx = (i - 1 + num_pop) % num_pop
            ...
            if len(best_fitness_history) > self.config.early_stopping_patience:
                ...
                if max(last_patience[:-1]) >= last_patience[-1] - 1e-6:
                    ...
                    print(
                        f"Early stopping in Hybrid Gen {stage1_gens + gen:03d} due to lack of improvement."
                    )
                    break
