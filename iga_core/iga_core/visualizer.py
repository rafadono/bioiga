        ...
        if show_solid:
            ...
            for i, u in enumerate(u_vals):
                for j, v in enumerate(v_vals):
                    ...
                    idx_u = max(
                        0, min(int(u * design.densities.shape[0]), design.densities.shape[0] - 1)
                    )
                    idx_v = max(
                        0, min(int(v * design.densities.shape[1]), design.densities.shape[1] - 1)
                    )
                    Dens[i, j] = design.densities[idx_u, idx_v]

            mesh = ax.pcolormesh(
                X, Y, Dens, cmap="gray_r", shading="auto", alpha=0.85, vmin=0, vmax=1
            )
