...
from .boundary import FixedSupport, LoadCase, PeriodicBoundary, PointLoad
...
def run_hole_experiment(grid_size=None, refine=False):
    ...
    best_design = studio.optimize(
        base_geometry, strategy="topology", void_mask=void_mask, solid_mask=solid_mask
    )
    print(
        f"[METRICA] Placa con Agujero Central - Tiempo de ejecucion: {best_design.execution_time:.2f}s"
    )
...
def run_cantilever_experiment(grid_size=None, refine=False):
    ...
    load_case = (
        LoadCase().add(FixedSupport(left_nodes)).add(PointLoad(bottom_right_node, fy=-5000.0))
    )
    studio = IGAOptimizer(solver, config)
...
def run_bandgap_experiment(grid_size=None, refine=False):
    ...
    print(
        f"[METRICA] Metamaterial Bandgap - Tiempo de ejecucion: {best_design.execution_time:.2f}s"
    )
...
def run_stress_experiment(grid_size=None, refine=False):
    ...
    load_case = (
        LoadCase().add(FixedSupport(top_left_nodes)).add(PointLoad(bottom_right_nodes, fy=-2000.0))
    )
    ...
    for est in estrategias:
        ...
        if est == "legacy":
            ...
            config.stress_penalty_factor = (
                10.0  # Debido a que la complacencia esta normalizada a ~1.0
            )
            config.p_norm_exponent = 8
        ...
        def custom_evaluate(design, orig_evaluate=orig_evaluate, load_case=load_case):
        ...
        U = solver.solve_statics(K_final, F_final)
        von_mises = solver.kernel.compute_von_mises_stress(
            best_design.geometry, best_design.densities, U
        )
        ...
        print(
            f"  [Resultado {est}] Compliance: {best_design.compliance:.4e} | Max Stress: {max_stress / 1e6:.2f} MPa | Vol: {best_design.volume:.3f}"
        )
        ...
        plt.plot(
            best_design.pop_fitness_histories[0],
            label=f"Estrategia {est.replace('_', ' ').capitalize()}",
            lw=2,
        )
...
def run_thesis_experiment(grid_size=None, refine=False):
    ...
    def evaluate_bandgap_brillouin(design):
        ...
        for kx, ky in path_k:
            freqs = solver.solve_bloch_frequencies(
                K_culled, M_culled, kx, ky, g_u, g_v, num_modes=4, K_is_culled=True
            )
            if len(freqs) >= 4:
    ...
    best_mbpso = studio_mbpso.optimize_mbpso(
        base_geometry, strategy="topology", tf_type="V", is_time_varying=True
    )
    ...
    best_msmbpso = studio_msmbpso.optimize_msmbpso(
        base_geometry, strategy="topology", tf_type="V", is_time_varying=True
    )
    ...
    best_hibrido = studio_hibrido.optimize_hybrid(
        base_geometry,
        strategy="topology",
        stage1_gens=8,
        stage2_gens=12,
    ...
    print("Estadísticas de la Caché de Evaluaciones:")
    ...
    msmbpso_es = (
        f"SI (Gen {best_msmbpso.stopped_at_generation})" if best_msmbpso.early_stopped else "NO"
    )
    hibrido_es = (
        f"SI (Gen {best_hibrido.stopped_at_generation})" if best_hibrido.early_stopped else "NO"
    )

    print(
        f"MBPSO    - Bandgap Máximo: {best_mbpso.compliance:.2f} Hz | Volumen: {best_mbpso.volume:.3f} | Tiempo: {time_mbpso:.2f}s | Early Stop: {mbpso_es}"
    )
    print(
        f"MpGA     - Bandgap Máximo: {best_mpga.compliance:.2f} Hz | Volumen: {best_mpga.volume:.3f} | Tiempo: {time_mpga:.2f}s | Early Stop: {mpga_es}"
    )
    print(
        f"MS-MBPSO - Bandgap Máximo: {best_msmbpso.compliance:.2f} Hz | Volumen: {best_msmbpso.volume:.3f} | Tiempo: {time_msmbpso:.2f}s | Early Stop: {msmbpso_es}"
    )
    print(
        f"HIBRIDO  - Bandgap Máximo: {best_hibrido.compliance:.2f} Hz | Volumen: {best_hibrido.volume:.3f} | Tiempo: {time_hibrido:.2f}s | Early Stop: {hibrido_es}"
    )
    print("=" * 60)
    ...
    resumen = {
        ...
        "resultados_optimizacion": {
            "MBPSO": {
                ...
                "generacion_parada": int(best_mbpso.stopped_at_generation)
                if best_mbpso.stopped_at_generation is not None
                else None,
            },
            "MpGA": {
                ...
                "generacion_parada": int(best_mpga.stopped_at_generation)
                if best_mpga.stopped_at_generation is not None
                else None,
            },
            "MS-MBPSO": {
                ...
                "generacion_parada": int(best_msmbpso.stopped_at_generation)
                if best_msmbpso.stopped_at_generation is not None
                else None,
            },
            "HIBRIDO": {
                ...
                "generacion_parada": int(best_hibrido.stopped_at_generation)
                if best_hibrido.stopped_at_generation is not None
                else None,
            },
        },
    }
    ...
    for design, title in [
        (best_mbpso, title_mbpso),
        (best_mpga, title_mpga),
        (best_msmbpso, title_msmbpso),
        (best_hibrido, title_hibrido),
    ]:
        ...
        for kx, ky in path_k:
            freqs = solver.solve_bloch_frequencies(
                K_opt_culled, M_opt_culled, kx, ky, g_u, g_v, num_modes=5, K_is_culled=True
            )
        ...
        K_stress, M_stress, _ = solver.assemble_system(
            design.geometry, design.densities, build_mass=True
        )
        ...
        print(
            f"  [OK] Esfuerzo Von Mises Modal Maximo para {title} (Frecuencia: {frequencies[elastic_mode_idx]:.2f} Hz): {max_vm:.4e} Pa"
        )
...
def run_thesis_validation_experiment(grid_size=None, refine=False):
    ...
    print(
        "| Grid | w1-w3 (Rígidos) | w4 (Bending 1) | w5 (Bending 2) | w6 (Bending 3) | w7-w8 (Bending 4-5) |"
    )
    print(
        "|------|------------------|----------------|----------------|----------------|---------------------|"
    )
    ...
    for g in grids:
        ...
        IGAGeometry(3, 3, knot_u, knot_v, ctrl_pts)
        ...
        print(
            f"| {g:2d}x{g:<2d} | 0.0000 - 0.0000  | {w_bending[0]:.4f} Hz     | {w_bending[1]:.4f} Hz     | {w_bending[2]:.4f} Hz     | {w_bending[3]:.4f} Hz          |"
        )
...
def run_plate_transfer_functions_study(grid_size=None, refine=False):
    ...
    for label, tf_type, is_time_varying in tf_configs:
        ...
        best_design = studio.optimize_mbpso(
            base_geometry, strategy="topology", tf_type=tf_type, is_time_varying=is_time_varying
        )
        elapsed = time.time() - start_time

        # Calcular esfuerzos Von Mises modales
        K_stress, M_stress, _ = solver.assemble_system(
            best_design.geometry, best_design.densities, build_mass=True
        )
        K_culled, M_culled = solver.cull_void_dofs(K_stress, M_stress, g_u, g_v)
        frequencies, eigenvectors = solver.solve_vibrations(K_culled, M_culled, num_modes=8)
        elastic_mode_idx = 3
        phi = eigenvectors[:, elastic_mode_idx]
        vm_stress = solver.kernel.compute_von_mises_stress(
            best_design.geometry, best_design.densities, phi
        )
        max_vm = np.max(vm_stress)
        ...
        title_placa = (
            f"Tesis_Placa_TF_{label.replace(' ', '_').replace('(', '').replace(')', '')}_{grid}"
        )
    ...
    plt.title(
        f"Convergencia de Fitness de Funciones de Transferencia en Placas IGA ({grid}x{grid})",
        fontsize=12,
        fontweight="bold",
    )
...
def run_thesis_v2_experiment(grid_size=None, refine=False):
    ...
    print(
        "=== REPRODUCIENDO TESIS DE MAGISTER VERSION 2 (Optimizacion con Restriccion de Esfuerzos) ==="
    )
    config, _, solver = _create_base_config()
    p = 3
    if grid_size is None:
    ...
    def evaluate_bandgap_brillouin_v2(design):
        strategy = getattr(design, "stress_strategy", None) or getattr(
            config, "stress_strategy", "strategy_1"
        )
        ...
        von_mises = solver.kernel.compute_von_mises_stress(
            design.geometry, design.densities, phi_scaled
        )
        ...
        if strategy == "legacy":
            max_stress = np.max(von_mises)
            stress_penalty = (
                (max_stress - yield_strength) * 1e3 if max_stress > yield_strength else 0.0
            )
            vol = np.mean(design.densities)
            ...
            if (
                not hasattr(evaluate_bandgap_brillouin_v2, "base_stress")
                or evaluate_bandgap_brillouin_v2.base_stress is None
            ):
                solid_densities = np.ones(design.densities.shape)
                K_base, M_base, _ = solver.assemble_system(
                    design.geometry, solid_densities, build_mass=True
                )
                K_base_culled, M_base_culled = solver.cull_void_dofs(K_base, M_base, g_u, g_v)
                frequencies_base, eigenvectors_base = solver.solve_vibrations(
                    K_base_culled, M_base_culled, num_modes=5
                )
                ...
                von_mises_base = solver.kernel.compute_von_mises_stress(
                    design.geometry, solid_densities, phi_base_scaled
                )
                p = getattr(config, "p_norm_exponent", 8)
                evaluate_bandgap_brillouin_v2.base_stress = (
                    np.mean((von_mises_base / sigma_adm) ** p)
                ) ** (1.0 / p) * sigma_adm
                print(
                    f"[Strategy 2] Bandgap Base Case Reference Calculated: S0 = {evaluate_bandgap_brillouin_v2.base_stress:.4e} Pa"
                )
    ...
    for alg, est, label in casos:
        ...
        if max_disp > 1e-12:
            ...
            phi_scaled = phi
        von_mises = solver.kernel.compute_von_mises_stress(
            best_design.geometry, best_design.densities, phi_scaled
        )
        ...
        print(
            f"  [Resultado {label}] Bandgap: {best_design.compliance:.2f} Hz | Max Stress: {max_vm / 1e6:.2f} MPa | Vol: {best_design.volume:.3f}"
        )
...
def run_thesis_v2_heterogeneous_experiment(grid_size=None, refine=False):
    ...
    def evaluate_bandgap_brillouin_v2(design):
        ...
        if strategy == "no_stress":
            ...
            if (
                ...
                p_val = getattr(config, "p_norm_exponent", 8)
                evaluate_bandgap_brillouin_v2.base_stress = (
                    np.mean((von_mises_base / sigma_adm) ** p_val)
                ) ** (1.0 / p_val) * sigma_adm
                print(
                    f"[Strategy 2] Bandgap Base Case Reference Calculated: S0 = {evaluate_bandgap_brillouin_v2.base_stress:.4e} Pa"
                )
    ...
    for alg, label_key, is_het, est, label in casos:
        ...
        print(
            f"  [Resultado {alg} - {label_key}] Bandgap: {best_design.compliance:.2f} Hz | Max Stress: {max_vm / 1e6:.2f} MPa | Vol: {best_design.volume:.3f}"
        )
...
def run_thesis_v2_bottleneck_experiment(grid_size=None, refine=False):
    ...
    if os.path.exists(csv_path):
        try:
            ...
            df_prev = df_prev[
                ~df_prev["Configuracion"].str.contains(
                    "Bottleneck|Secuencial", case=False, na=False
                )
            ]
    ...
    for alg, label_key, est, label in casos_bottleneck:
        ...
        print(
            f"  [Resultado Bottleneck] Bandgap: {best_design.compliance:.2f} Hz | Max Stress: {max_vm / 1e6:.2f} MPa | Vol: {best_design.volume:.3f}"
        )
        ...
        plt.plot(
            best_design.best_compliance_history, label=f"{alg} ({label_key})", lw=2, linestyle="--"
        )
...
def run_thesis_v2_statistical_experiment(grid_size=None, refine=False):
    ...
    casos = [
        ...
        (
            "MS-MBPSO",
            "Secuencial S1 (Bottleneck)",
            False,
            True,
            "strategy_1",
            "MS-MBPSO Bottleneck S1",
        ),
        (
            "MS-MBPSO",
            "Secuencial S2 (Bottleneck)",
            False,
            True,
            "strategy_2",
            "MS-MBPSO Bottleneck S2",
        ),
        # HIBRIDO
    ...
    for alg, label_key, is_het, is_delayed, est, _label in casos:
        print("\n" + "=" * 80)
        ...
        for run_idx in range(N_RUNS):
            ...
            print(
                f"  Corrida {run_idx:02d} | Bandgap: {bg_val:.2f} Hz | Esfuerzo: {stress_val:.2f} MPa | Tiempo: {elapsed:.1f}s"
            )
