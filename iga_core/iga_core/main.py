...
def main():
    ...
    help_text += (
        "  bandgap              - Design of acoustic metamaterials (vibrational analysis).\n"
    )
    help_text += (
        "  stress               - Von Mises stress control to avoid geometric singularities.\n"
    )
    help_text += (
        "  robust               - Optimization against CNC manufacturing uncertainty (Culling).\n"
    )
    ...
    help_text += (
        "  thesis_v2_bottleneck - Simulates the longevity bottleneck hypothesis on plates.\n"
    )
    help_text += "  thesis_v2_stat       - Statistical study across 13 optimization cases (13 cases x 10 runs).\n"
    help_text += (
        "  thesis_validation    - Runs validation scenarios (frequencies and bandgap diagrams).\n"
    )
    help_text += "  tuning               - Tunes hyperparameters (pop_size, mutation, early stopping) using Optuna.\n"
    help_text += (
        "  tf_plate             - Comparative study of 8 transfer functions on 16x16 IGA plates.\n"
    )
    ...
    parser.add_argument(
        "--elements", type=int, default=16, help="Number of subdivisions per axis. Default: 16."
    )
    parser.add_argument(
        "--refine",
        action="store_true",
        help="Applies Boehm's knot refinement at the corners of the plate.",
    )
