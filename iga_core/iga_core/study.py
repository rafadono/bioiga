...
def run_study(algorithm_name="mpga", n_trials=15):
    ...
    # Brillouin path
    path_k = []
    for f in np.linspace(0, np.pi, 4): path_k.append((f, 0.0))
    for f in np.linspace(0, np.pi, 4): path_k.append((np.pi, f))
    for f in np.linspace(np.pi, 0, 4): path_k.append((f, f))
    ...
    def objective(trial):
        ...
        def evaluate_bandgap_brillouin(design):
            ...
            band4_freqs = []
            for kx, ky in path_k:
                freqs = solver.solve_bloch_frequencies(K_culled, M_culled, kx, ky, grid, grid, num_modes=4, K_is_culled=True)
                if len(freqs) >= 4:
    ...
    print(f"  Computation time of the best: {best_trial.user_attrs.get('execution_time', 0.0):.2f}s")
