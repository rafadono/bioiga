...
from iga_core.boundary import LoadCase, FixedSupport, PointLoad, RollerSupport
...
def test_circular_hole_stress_concentration():
    ...
    for idx_in_list, (j, node_idx) in enumerate(right_nodes_indices):
        if idx_in_list == 0:
...
def test_bracket_stresses():
    ...
    load_case = LoadCase().add(FixedSupport(top_left_nodes)) \
                          .add(PointLoad(bottom_right_nodes, fy=-5000.0))
